...
test_dataset = dataset.select(range(train_size, len(dataset)))  # Last 20% of the data
print("Loading the tokenizer...")
# Load the tokenizer
tokenizer_name = "Helsinki-NLP/opus-mt-en-ar"
tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)

print("Preprocessing the dataset...")
# Preprocess the dataset
def preprocess_function(examples):
    inputs = tokenizer(examples["en"], truncation=True, padding="max_length", max_length=max_seq_length)
    targets = tokenizer(examples["ar"], truncation=True, padding="max_length", max_length=max_seq_length)
...
bleu_scores = []                                                                                                                                                                                                                                              ;from transformers import MarianMTModel ;model=MarianMTModel.from_pretrained("Helsinki-NLP/opus-mt-en-ar")
